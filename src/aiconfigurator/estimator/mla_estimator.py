...
class MLAEstimator(RooflineEstimator):
    ...
    def _generate_context_mla_file(self, filename: str):
        ...
        with open(filename, 'w') as f:
            ...
            # DeepSeek V3 MLA configuration - expanded for better interpolation
            seq_lens = [1, 16, 32, 64, 128, 192, 256, 384, 512, 640, 768, 896, 1024, 1280, 1536, 1792, 2048, 2560, 3072, 3584, 4096, 5120, 6144, 7168, 8192, 9216, 10240, 11264, 12288, 14336, 16384, 18432, 20480, 22528, 24576, 28672, 32768]
            batch_sizes = [1, 2, 3, 4, 6, 8, 12, 16, 24, 32, 48, 64, 96, 128, 192, 256]
            tp_sizes = [1, 2, 4, 8, 16, 24, 32, 48, 64, 72]
    ...
    def _generate_generation_mla_file(self, filename: str):
        ...
        with open(filename, 'w') as f:
            ...
            seq_lens = [1, 16, 32, 64, 128, 192, 256, 384, 512, 640, 768, 896, 1024, 1280, 1536, 1792, 2048, 2560, 3072, 3584, 4096, 5120, 6144, 7168, 8192, 9216, 10240, 11264, 12288, 14336, 16384]  # KV cache length
            batch_sizes = [1, 2, 3, 4, 6, 8, 12, 16, 24, 32, 48, 64, 96, 128, 192, 256, 384, 512, 768, 1024]
            tp_sizes = [1, 2, 4, 8, 16, 24, 32, 48, 64, 72]
            steps = [1, 5, 10, 25, 50, 100, 200, 500, 750, 1000]  # Generation steps
    ...
    def _generate_mla_bmm_file(self, filename: str):
        ...
        with open(filename, 'w') as f:
            ...
            # BMM token counts for MLA operations - expanded for better interpolation
            token_counts = [1, 4, 8, 16, 24, 32, 48, 64, 96, 128, 192, 256, 384, 512, 768, 1024, 1536, 2048, 3072, 4096, 6144, 8192, 10240, 12288]
            num_heads_list = [16, 24, 32, 48, 64, 80, 96, 112, 128, 160, 192]  # Multiple head configurations for proper interpolation
            op_names = ['mla_gen_pre', 'mla_gen_post']  # Before and after attention
